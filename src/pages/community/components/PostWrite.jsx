...
function PostWrite({ onSubmit, onCancel }) {
  ...
  const addFiles = (fileList) => {
    ...
    const deduped = incoming.filter((f) => !existsKey.has(`${f.name}-${f.size}`));
    ...
    >
      ...
      >
        <div className={styles.modalHeader || ""}>
          <h2 id="postwrite-title" className={styles.modalTitle || ""}>글쓰기</h2>
          <button
            type="button"
            ...
            {images.length > 0 && (
              <div className={styles.thumbGrid || ""}>
                {images.map((file, idx) => {
                  ...
                  return (
                    <div className={styles.thumbItem || ""} key={`${file.name}-${idx}`}>
                      <img
                        src={url}
                        alt={file.name}
      ...
      <style>{`
        ...
        .${styles.modalTitle || "title-fallback"}{ font-size:18px; font-weight:700; color:#1f2a44; }
        ...
        .${styles.formLabel || "label-fallback"}{ display:block; margin-bottom:8px; color:#1f2a44; font-weight:600; }
        ...
        .${styles.dropzoneInner || "dz-inner-fallback"}{ display:flex; flex-direction:column; gap:8px; align-items:center; }
        .${styles.uploadIcon || "up-ic-fallback"}{ font-size:20px; }
        .${styles.uploadText || "up-tx-fallback"}{ color:#667085; }
        .${styles.thumbGrid || "grid-fallback"}{ display:grid; grid-template-columns:repeat(auto-fill, minmax(88px,1fr)); gap:10px; margin-top:12px; }
        .${styles.thumbItem || "item-fallback"}{ position:relative; border:1px solid #eee; border-radius:10px; overflow:hidden; }
        .${styles.thumbImg || "img-fallback"}{ width:100%; height:88px; object-fit:cover; display:block; }
        ...
        .${styles.checkboxLabel || "chklb-fallback"}{ display:flex; gap:8px; align-items:center; color:#475467; }
        .${styles.modalFooter || "footer-fallback"}{ display:flex; justify-content:flex-end; gap:8px; padding-top:8px; }
