...
function loadUserOnce() {
  ...
  const deptRaw =
    base.deptId ?? base.user_dept ?? base.department ?? base.department_id ?? base.department_name ?? base.departmentId ?? null;
...
const LANG_CODE = { 한국어: "ko", English: "en", 日本語: "ja"};
...
export default function Header() {
  ...
  const topLeft = isAdmin
    ? { label: t("header.title"), iconName: "shield", color: "#ea580c" }
    : { label: dept?.label || t("header.noDept"), iconName: dept?.icon || "shield", color: "#2563eb" };
          ...
          <div className="dropdown" ref={notifMenuRef}>
            ...
            {notifOpen && (
              <ul className="menu" role="menu" style={{ minWidth: 320, paddingTop: 8, paddingBottom: 8 }}>
                ...
                {notifs.length === 0 ? (
                  <li role="menuitem" style={{ padding: "12px" }}>{t("header.noNewNotifs")}</li>
                ) : (
                  <>
                    <div
                      style={{
                        ...
                        return (
                          ...
                          >
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                              ...
                              <button
                                type="button"
                                aria-label={t("header.deleteNotif")}
                    ...
                    <li role="presentation" style={{ padding: "8px 12px", background: "transparent" }}>
                      <div style={{ display: "flex", justifyContent: "flex-end" }}>
                        <div
                          role="group"
                          aria-label={t("header.notifActions")}
        ...
        <div className="dropdown" ref={langMenuRef}>
          ...
          {langOpen && (
            <ul className="menu" role="menu">
              {Object.keys(LANG_CODE).map((l) => (
                <li
                  key={l}
                  role="menuitem"
                  onClick={() => {
                    setLang(l);
                    i18n.changeLanguage(LANG_CODE[l]);
                    setLangOpen(false);
                  }}
                >
                  {l}
